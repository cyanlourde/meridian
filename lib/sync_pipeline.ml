...
(** Collect a batch of header points via chain-sync.
    Returns (points, at_tip, tip_info) where at_tip is true if
    MsgAwaitReply was received before the batch was full. *)
let collect_headers net ~batch_size ~should_stop =
  let points = ref [] in
  let at_tip = ref false in
  let tip_info = ref (0L, 0L) in
  let count = ref 0 in
  let running = ref true in
  while !running && not (should_stop ()) && !count < batch_size do
    match Network.request_next net with
    | Error e -> running := false; points := [];
      (* Signal error by returning empty + error indicator *)
      tip_info := (-1L, -1L);
      ignore e
    | Ok (Roll_forward { header; tip }) ->
      ...
      (match extract_point header with
       ...
       | _ -> ())
    | Ok (Roll_backward _) -> ()  (* continue collecting *)
    | Ok Await_reply ->
      at_tip := true;
      running := false
  done;
...
let fetch_and_store net store points =
  ...
  | [] | [_] -> Ok 0
  | _ ->
    let from_pt = List.hd points in
    let to_pt = List.nth points (List.length points - 1) in
    ...
    | Batch_started ->
      ...
      while !streaming do
        ...
        | Ok (Some block_bytes) ->
          if !block_idx < Array.length pts_arr then begin
            (match pts_arr.(!block_idx) with
