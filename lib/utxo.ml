(* UTXO set management for Cardano.

   Reference: Shelley formal ledger spec (SL-D5), Section 9 "UTxO"

   The UTxO (Unspent Transaction Output) set tracks all outputs that
   have been created but not yet consumed. Each entry is keyed by a
   TxIn (transaction hash + output index) and maps to a TxOut.

   The UTXO transition rule:
     utxo' = (utxo \ inputs(tx)) ∪ outputs(tx)
   i.e., remove consumed inputs, add new outputs.

   Validation checks per the Shelley spec:
   - All inputs must exist in the current UTxO set
   - No input may be consumed twice in the same transaction
   - Consumed value = produced value (inputs + withdrawals = outputs + fee + deposits)
   - Fee must meet minimum (fee >= minFee tx)
   - Output values must meet minimum (lovelace >= minUTxOValue)
   - TTL must not have passed *)

(* ================================================================ *)
(* UTxO key: transaction input reference                             *)
(* ================================================================ *)

module TxIn = struct
  type t = {
    tx_hash : bytes;  (** 32-byte transaction hash *)
    tx_index : int;   (** Output index within that transaction *)
  }

  let compare a b =
    let c = Bytes.compare a.tx_hash b.tx_hash in
    if c <> 0 then c else Int.compare a.tx_index b.tx_index

  let of_decoder (inp : Tx_decoder.tx_input) =
    { tx_hash = inp.ti_tx_hash; tx_index = Int64.to_int inp.ti_index }

  let equal a b = compare a b = 0

  let to_string t =
    let hex = Buffer.create 16 in
    for i = 0 to min 7 (Bytes.length t.tx_hash - 1) do
      Buffer.add_string hex (Printf.sprintf "%02x" (Bytes.get_uint8 t.tx_hash i))
    done;
    Printf.sprintf "%s..#%d" (Buffer.contents hex) t.tx_index
end

(* ================================================================ *)
(* UTxO value: transaction output                                    *)
(* ================================================================ *)

module TxOut = struct
  type t = {
    address : bytes;
    lovelace : int64;
    has_multi_asset : bool;
    has_datum : bool;
    has_script_ref : bool;
  }

  let of_decoder (out : Tx_decoder.tx_output) =
    { address = out.to_address;
      lovelace = out.to_lovelace;
      has_multi_asset = out.to_has_multi_asset;
      has_datum = out.to_has_datum;
      has_script_ref = out.to_has_script_ref }
end

(* ================================================================ *)
(* UTxO set (in-memory, Map-based)                                   *)
(* ================================================================ *)

module TxInMap = Map.Make(TxIn)

type utxo_set = {
  mutable entries : TxOut.t TxInMap.t;
  mutable size : int;
}

let create () = { entries = TxInMap.empty; size = 0 }

let size s = s.size

let mem s txin = TxInMap.mem txin s.entries

let find s txin = TxInMap.find_opt txin s.entries

let add s txin txout =
  if not (TxInMap.mem txin s.entries) then
    s.size <- s.size + 1;
  s.entries <- TxInMap.add txin txout s.entries

let remove s txin =
  if TxInMap.mem txin s.entries then begin
    s.entries <- TxInMap.remove txin s.entries;
    s.size <- s.size - 1
  end

(** Iterate over all entries. *)
let iter f s =
  TxInMap.iter (fun k v -> f k v) s.entries

(** Total lovelace in the entire UTxO set. *)
let total_lovelace s =
  TxInMap.fold (fun _k v acc -> Int64.add acc v.TxOut.lovelace) s.entries 0L

(* ================================================================ *)
(* Transaction validation                                            *)
(* ================================================================ *)

type validation_error =
  | Input_not_in_utxo of TxIn.t
  | Duplicate_input of TxIn.t
  | Insufficient_fee of { required : int64; actual : int64 }
  | Value_not_conserved of { consumed : int64; produced : int64 }
  | Output_too_small of { index : int; lovelace : int64; minimum : int64 }
  | Expired_ttl of { slot : int64; ttl : int64 }
  | Empty_inputs
  | Empty_outputs

let error_to_string = function
  | Input_not_in_utxo txin ->
    Printf.sprintf "input not in UTxO: %s" (TxIn.to_string txin)
  | Duplicate_input txin ->
    Printf.sprintf "duplicate input: %s" (TxIn.to_string txin)
  | Insufficient_fee { required; actual } ->
    Printf.sprintf "fee %Ld below minimum %Ld" actual required
  | Value_not_conserved { consumed; produced } ->
    Printf.sprintf "value not conserved: consumed %Ld != produced %Ld" consumed produced
  | Output_too_small { index; lovelace; minimum } ->
    Printf.sprintf "output %d too small: %Ld < %Ld" index lovelace minimum
  | Expired_ttl { slot; ttl } ->
    Printf.sprintf "TTL expired: slot %Ld > TTL %Ld" slot ttl
  | Empty_inputs -> "transaction has no inputs"
  | Empty_outputs -> "transaction has no outputs"

(** Minimum fee calculation per Shelley spec: fee = a * size + b
    where a = minFeeA (per-byte), b = minFeeB (constant).
    We estimate tx size from input/output counts. *)
let min_fee ~min_fee_a ~min_fee_b ~tx_size =
  Int64.add (Int64.mul min_fee_a (Int64.of_int tx_size)) min_fee_b

(** Validate a single transaction against the current UTxO set.
    Returns list of all validation errors (empty = valid).

    Parameters:
    - [utxo]: current UTxO set
    - [tx]: decoded transaction
    - [current_slot]: current slot for TTL check
    - [min_fee_a]: linear fee coefficient (default 44)
    - [min_fee_b]: constant fee (default 155381)
    - [min_utxo_value]: minimum lovelace per output (default 1000000)
    - [tx_size_estimate]: estimated serialized tx size in bytes *)
let validate_tx ?(min_fee_a = 44L) ?(min_fee_b = 155381L)
    ?(min_utxo_value = 1000000L) ?(tx_size_estimate = 300)
    ~utxo ~current_slot (tx : Tx_decoder.decoded_tx) =
  let errors = ref [] in
  let add e = errors := e :: !errors in

  (* Check non-empty inputs/outputs *)
  if tx.dt_inputs = [] then add Empty_inputs;
  if tx.dt_outputs = [] then add Empty_outputs;

  (* Check all inputs exist in UTxO and are not duplicated *)
  let seen_inputs = Hashtbl.create (List.length tx.dt_inputs) in
  let consumed_value = ref 0L in
  List.iter (fun (inp : Tx_decoder.tx_input) ->
    let txin = TxIn.of_decoder inp in
    if Hashtbl.mem seen_inputs (txin.tx_hash, txin.tx_index) then
      add (Duplicate_input txin)
    else begin
      Hashtbl.replace seen_inputs (txin.tx_hash, txin.tx_index) ();
      match find utxo txin with
      | None -> add (Input_not_in_utxo txin)
      | Some out ->
        consumed_value := Int64.add !consumed_value out.TxOut.lovelace
    end
  ) tx.dt_inputs;

  (* Check fee meets minimum *)
  let required_fee = min_fee ~min_fee_a ~min_fee_b ~tx_size:tx_size_estimate in
  if tx.dt_fee < required_fee then
    add (Insufficient_fee { required = required_fee; actual = tx.dt_fee });

  (* Check output minimum values *)
  List.iteri (fun i (out : Tx_decoder.tx_output) ->
    if out.to_lovelace < min_utxo_value then
      add (Output_too_small { index = i; lovelace = out.to_lovelace;
                              minimum = min_utxo_value })
  ) tx.dt_outputs;

  (* Check TTL *)
  (match tx.dt_ttl with
   | Some ttl when Int64.compare current_slot ttl > 0 ->
     add (Expired_ttl { slot = current_slot; ttl })
   | _ -> ());

  (* Check value conservation:
     consumed (sum of input values) = produced (sum of outputs + fee) *)
  let produced_value =
    List.fold_left (fun acc (out : Tx_decoder.tx_output) ->
      Int64.add acc out.to_lovelace
    ) 0L tx.dt_outputs
  in
  let total_produced = Int64.add produced_value tx.dt_fee in
  if !consumed_value > 0L && not (Int64.equal !consumed_value total_produced) then
    add (Value_not_conserved { consumed = !consumed_value; produced = total_produced });

  List.rev !errors

(* ================================================================ *)
(* UTxO transition: apply a transaction                              *)
(* ================================================================ *)

(** Apply a validated transaction to the UTxO set.
    Removes consumed inputs, adds new outputs.
    [tx_hash] is the hash of this transaction (needed to key new outputs). *)
let apply_tx utxo ~tx_hash (tx : Tx_decoder.decoded_tx) =
  (* Remove consumed inputs *)
  List.iter (fun (inp : Tx_decoder.tx_input) ->
    remove utxo (TxIn.of_decoder inp)
  ) tx.dt_inputs;
  (* Add new outputs *)
  List.iteri (fun i (out : Tx_decoder.tx_output) ->
    let txin = TxIn.{ tx_hash; tx_index = i } in
    let txout = TxOut.of_decoder out in
    add utxo txin txout
  ) tx.dt_outputs

(** Apply all transactions in a decoded block to the UTxO set.
    Returns the number of transactions applied and any validation errors. *)
let apply_block utxo ~current_slot ~tx_hash_fn
    (block : Block_decoder.decoded_block) =
  let total_errors = ref [] in
  let applied = ref 0 in
  List.iter (fun tx_cbor ->
    match Tx_decoder.decode_transaction ~era:block.db_era tx_cbor with
    | Error _e -> ()
    | Ok tx ->
      let errors = validate_tx ~utxo ~current_slot tx in
      if errors <> [] then
        total_errors := errors @ !total_errors
      else begin
        let tx_hash = tx_hash_fn tx_cbor in
        apply_tx utxo ~tx_hash tx;
        incr applied
      end
  ) block.db_tx_raw;
  (!applied, List.rev !total_errors)
